...
function MoveTouch:BindContextActions()
    ...
    self._connectionUtil:trackConnection(
        ...
        self.jumpButton.InputEnded:Connect(function(inputObj)
    ...
    self._connectionUtil:trackConnection(
        MENU_OPEN_ACTION_NAME,
        GuiService.MenuOpened:Connect(function(inputObj)
            if (inputObj == self.jumpTouchObj) then
                self:resetJump()
            end
            if (inputObj == self.dashTouchObj) then
...
function MoveTouch:create(parentFrame: GuiBase2d)
    ...
    local resizeThumbstick do
        ...
        local function layoutThumbstickFrame(portraitMode: boolean)
            if portraitMode then
                self.thumbstickFrame.Size = UDim2.new(1, SAFE_AREA_INSET_MAX, 0.4, SAFE_AREA_INSET_MAX)
                self.thumbstickFrame.Position = UDim2.new(0, -SAFE_AREA_INSET_MAX, 0.6, 0)
            else
                self.thumbstickFrame.Size = UDim2.new(0.4, SAFE_AREA_INSET_MAX, 2/3, SAFE_AREA_INSET_MAX)
                self.thumbstickFrame.Position = UDim2.new(0, -SAFE_AREA_INSET_MAX, 1/3, 0)
            end
        end
    ...
    local resizeJumpBtn do
        ...
        function resizeJumpBtn()
            ...
            self.jumpButton.Position = isSmallScreen and 
                UDim2.new(1, -(jumpButtonSize*1.5-10), 1, -jumpButtonSize - 20) or
        ...
        self.jumpButton.InputBegan:Connect(function(inputObject)
            if (self.jumpTouchObj or inputObject.UserInputType ~= Enum.UserInputType.Touch
                or inputObject.UserInputState ~= Enum.UserInputState.Begin) then
                return
            end

            self.jumpTouchObj = inputObject
